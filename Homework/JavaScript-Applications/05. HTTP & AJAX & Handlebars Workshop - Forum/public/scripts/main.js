...
$(() => { // on document ready
  ...
  contentContainer.on('click', '#btn-add-thread', (ev) => {
    ...
    data.threads.add(title)
        .then(/* add to UI */)
        .then(router.showMsg('Successfuly added the new thread', 'Success', 'alert-success'))
        .catch((err) => router.showMsg(JSON.parse(err.responseText).err, 'Error', 'alert-danger'));
  })
  ...
  contentContainer.on('click', '.btn-add-message', (ev) => {
    ...
    data.threads.addMessage(thId, msg)
        .then(/* add to UI */)
        .then(router.showMsg('Successfuly added the new mssagee', 'Success', 'alert-success'))
        .catch((err) => router.showMsg(JSON.parse(err.responseText).err, 'Error', 'alert-danger'));
  })
